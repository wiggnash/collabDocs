...
- [x] Filtering by `tag` works via query params (`tags__name`).
...
- [x] `stats/` returns correct version, comment, and contributor counts (aggregation, not Python loops).
- [x] `tags/` attaches the given tags and they appear on subsequent reads / tag filtering.
