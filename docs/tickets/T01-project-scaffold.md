...
The runnable Django + DRF project skeleton that every other ticket builds on. No business logic yet — just a server that starts, connects to PostgreSQL via environment variables, and exposes an empty `/api/` router.
...
- Configure `settings.py` to read DB config and `SECRET_KEY` from `.env` via `django-environ` (`env.db()` for `DATABASES`, `env.bool('DEBUG', ...)`) — **no hardcoded credentials**.
...
- Commit a `.env.example` with all required variable names (no real values), including the `DATABASE_URL` expected by `env.db()`.
...
- [ ] `uv run python manage.py migrate` connects to PostgreSQL successfully (Django's built-in tables apply).
